...
const defaultConfig: Required<ParticleConfig> = {
  count: 75, // 降低一半：150 -> 75
  minRadius: 2,
  maxRadius: 5,
  minSpeed: 0.2,
  maxSpeed: 0.8,
  colors: ['#00D9FF', '#8B5CF6', '#06B6D4', '#22D3EE', '#A78BFA', '#ffffff'], // 更新为新的配色
  connectionDistance: 200,
  connectionOpacity: 0.25,
  glowIntensity: 8, // 减弱发光：15 -> 8
};
...
export class Particle {
  ...
  constructor(
    ...
    this.baseOpacity = Math.random() * 0.4 + 0.3; // 降低基础透明度：0.4-1.0 -> 0.3-0.7
    this.opacity = this.baseOpacity;
    this.color = config.colors[Math.floor(Math.random() * config.colors.length)];
    this.pulseSpeed = Math.random() * 0.03 + 0.01;
    this.pulseOffset = Math.random() * Math.PI * 2;
    this.glowSize = config.glowIntensity + Math.random() * 5; // 降低发光范围：10 -> 5
  }
  ...
  update(canvasWidth: number, canvasHeight: number, time: number): void {
    ...
    // Pulse effect (twinkling) - 减弱脉冲效果
    const pulse = Math.sin(time * this.pulseSpeed + this.pulseOffset);
    this.opacity = this.baseOpacity + pulse * 0.15; // 降低脉冲幅度：0.3 -> 0.15
    this.radius = this.baseRadius + pulse * 0.5; // 降低半径变化：1 -> 0.5
  }
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    gradient.addColorStop(0, this.hexToRgba(this.color, this.opacity * 0.8)); // 降低中心亮度
    gradient.addColorStop(0.3, this.hexToRgba(this.color, this.opacity * 0.3)); // 降低中间亮度：0.5 -> 0.3
    ...
    for (let i = 0; i < this.particles.length; i++) {
      for (let j = i + 1; j < this.particles.length; j++) {
        ...
        if (distance < this.config.connectionDistance) {
          ...
          // Create gradient line for connection - 使用新的配色
          const gradient = this.ctx.createLinearGradient(
            this.particles[i].x, this.particles[i].y,
            this.particles[j].x, this.particles[j].y
          );
          gradient.addColorStop(0, `rgba(139, 92, 246, ${opacity})`); // 紫色
          gradient.addColorStop(0.5, `rgba(0, 217, 255, ${opacity * 0.8})`); // 青色
          gradient.addColorStop(1, `rgba(139, 92, 246, ${opacity})`); // 紫色
